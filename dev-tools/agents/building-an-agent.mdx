...
<Steps>
  <Step title="Build a Plane App">
    Follow the [Build a Plane App](/dev-tools/build-plane-app) guide to understand OAuth flows, deployment, and webhook handling.
  </Step>
  <Step title="Get Your Bot Token">
    Complete the [Bot Token Flow](/dev-tools/build-plane-app#bot-token-flow-recommended-for-agents--automation) to obtain a `bot_token` for your agent. This token is used for all API calls.
  </Step>
  <Step title="Set Up Webhook Handling">
    Ensure your server can [receive and verify webhooks](/dev-tools/build-plane-app#webhook-payload-structure) from Plane.
  </Step>
</Steps>
