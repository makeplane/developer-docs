---
title: Create a workspace mapping
description: Create a workspace mapping with the Plane v2 REST API. Parameters, sparse `?fields=` responses, OAuth scopes, error codes, and code examples.
keywords: plane api v2, create a workspace mapping, workspace mappings, group sync workspace mappings create
---
...
<div class="api-endpoint-badge">
  ...
  <span class="path">/api/v2/workspaces/{slug}/group-sync/workspace-mappings/</span>
</div>
...
<CodePanel title="Create a workspace mapping" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>

```bash
curl -X POST \
  "https://api.plane.so/api/v2/workspaces/my-team/group-sync/workspace-mappings/" \
...
```javascript
const response = await fetch("https://api.plane.so/api/v2/workspaces/my-team/group-sync/workspace-mappings/", {
  method: "POST",
  headers: {
