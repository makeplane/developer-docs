---
title: List relation definitions
description: List relation definitions with the Plane v2 REST API. Parameters, sparse `?fields=` responses, OAuth scopes, error codes, and code examples.
keywords: plane api v2, list relation definitions, relation definitions, work item relation definitions list
---
...
Relation definitions are the workspace registry of typed work item relationships. List the relation definitions you can see. Results are scoped to the path and to what your token is allowed to read.
...
`workspaces.work_item_relation_definitions:read`
...
<CodePanel title="List relation definitions" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>
...
```javascript
const response = await fetch("https://api.plane.so/api/v2/workspaces/my-team/work-item-relation-definitions/", {
  method: "GET",
  headers: {
