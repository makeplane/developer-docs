...
description: List customer properties with the Plane v2 REST API. Parameters, sparse `?fields=` responses, OAuth scopes, error codes, and code examples.
keywords: plane api v2, list customer properties, customer properties, customer properties list
---
...
<CodePanel title="List customer properties" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>
...
```javascript
const response = await fetch("https://api.plane.so/api/v2/workspaces/my-team/customer-properties/", {
  method: "GET",
  headers: {
