...
description: Update a workflow state with the Plane v2 REST API. Parameters, sparse `?fields=` responses, OAuth scopes, error codes, and code examples.
keywords: plane api v2, update a workflow state, workflow states, workflow states partial update
---
...
<div class="api-endpoint-badge">
  ...
  <span class="path">/api/v2/workspaces/{slug}/projects/{project_id}/workflows/{workflow_id}/states/{pk}/</span>
</div>
...
<ApiParam name="allow_issue_creation" type="boolean" :required="false">
...
Requestable here: `allow_issue_creation`, `created_at`, `created_by_id`, `id`, `is_default`, `state_id`, `type`, `workflow_id`.
...
<CodePanel title="Update a workflow state" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>

```bash
curl -X PATCH \
  "https://api.plane.so/api/v2/workspaces/my-team/projects/4af68566-94a4-4eb3-94aa-50dc9427067b/workflows/9c1f0b3d-6d2e-4c9a-8b41-2f7e5a0d6c88/states/8f4c2b1e-0d3a-4f7b-9c21-6e5a8b7d4f13/" \
...
```json
{
  "type": "not_found",
  "code": "not_found",
  "detail": "No workflow state matches the given query."
}
```
