...
description: Update a webhook with the Plane v2 REST API. Parameters, sparse `?fields=` responses, OAuth scopes, error codes, and code examples.
keywords: plane api v2, update a webhook, webhooks, webhooks partial update
---
...
<div class="api-endpoint-badge">
  ...
  <span class="path">/api/v2/workspaces/{slug}/webhooks/{pk}/</span>
</div>
...
<ApiParam name="content_type" type="string" :required="false">
...
<ApiParam name="scopes" type="array of string" :required="false">

The scopes.

</ApiParam>

<ApiParam name="url" type="string (uri)" :required="false">
...
Requestable here: `content_type`, `created_at`, `created_by_id`, `id`, `is_active`, `name`, `scopes`, `url`, `version`.
...
<CodePanel title="Update a webhook" :languages="['cURL', 'Python', 'JavaScript']">
<template #curl>

```bash
curl -X PATCH \
  "https://api.plane.so/api/v2/workspaces/my-team/webhooks/9c1f0b3d-6d2e-4c9a-8b41-2f7e5a0d6c88/" \
...
```json
{
  "type": "not_found",
  "code": "not_found",
  "detail": "No webhook matches the given query."
}
```
