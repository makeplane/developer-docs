...
```bash
curl -X GET \
  "https://api.plane.so/api/v1/workspaces/my-workspace/initiatives/{initiative_id}/labels/?cursor=20:1:0&per_page=20" \
...
```javascript
const response = await fetch("https://api.plane.so/api/v1/workspaces/my-workspace/initiatives/{initiative_id}/labels/?cursor=20:1:0&per_page=20", {
  method: "GET",
  headers: {
