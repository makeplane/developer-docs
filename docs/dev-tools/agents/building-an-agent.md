...
2. **Get Your Bot Token** — Complete the [Bot Token Flow](/dev-tools/build-plane-app#bot-token-flow-recommended-for-agents--automation) to obtain a `bot_token` for your agent. This token is used for all API calls.

3. **Set Up Webhook Handling** — Ensure your server can [receive and verify webhooks](/dev-tools/build-plane-app#webhook-payload-structure) from Plane.
