...
1. **Delink licenses**
   Log in to Plane on your current server. Go to each paid workspace (such as One or Pro) and [delink the licenses](https://docs.plane.so/workspaces-and-users/upgrade-plan#delink-license-key). This will free up the licenses for activation on your new server. Ideally, you have just one paid workspace.
...
4. **Transfer backup files**
   Copy the `backups` folder from the old server (created in step 2) to the new server. Place the backup in the `/opt/plane` directory.
...
5. **Restore data**
   ...
   prime-cli restore
   ```
   Follow the prompts during the restore process to ensure everything is set up correctly.
