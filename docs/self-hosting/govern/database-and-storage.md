...
# Configure external services <EditionBadge edition="commercial" />
...
- `External Postgres URL`  
  ...
  _Default_: `PostgreSQL 15.7` in the bundled Docker container.
...
- `External Redis URL`  
  ...
  _Default_: `Valkey 7.2` (Redis-compatible) in the bundled Docker container
...
On the Community Edition, set `DATABASE_URL`, `REDIS_URL`, `AMQP_URL`, and the `AWS_*` variables in `plane-app/plane.env` and restart with `./setup.sh restart`. See [Community Edition → Use external services](/self-hosting/community/manage#use-external-services).
