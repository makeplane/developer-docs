...
| **WEB_URL**              | The complete base URL for the web application including protocol (e.g., `https://<your-domain>`).                             | http://localhost          |
...
| **SITE_ADDRESS**  | The site address the built-in Caddy proxy listens on. `<your-domain>`: Caddy serves that hostname and provisions a Let's Encrypt certificate (ports 80/443 and DNS required). `http://<your-domain>`: plain HTTP for that hostname. `:80`: plain HTTP for any hostname, for use behind an [external reverse proxy](/self-hosting/govern/reverse-proxy). | localhost:80                                   |
