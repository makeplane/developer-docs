...
- The issue may also be caused by using an older version of Docker Compose (`docker-compose`). To fix, install the latest version of Docker Compose (`docker compose`) and remove the old version.
...
To resolve, ensure the database is hosted on a network-accessible server rather than localhost. Update the database URL to reflect the correct server address. See [how to configure external db](/self-hosting/govern/database-and-storage).
